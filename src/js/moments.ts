(function() {
  ...
  function lbRenderThumbs() {
    ...
    if (lbPhotos.length <= 1) { lbBottom.style.display = 'none'; return; }
    ...
    lbPhotos.forEach(function(src, i) {
      ...
      img.alt = '';
      (function(idx: number) {
        img.addEventListener('click', function() { lbShow(idx); });
      })(i);
  ...
  document.querySelectorAll('[data-moment-photos]').forEach(function(grid) {
    ...
    btns.forEach(function(btn, idx) {
      btn.addEventListener('click', function() {
        lbPhotos = [];
        btns.forEach(function(b) { lbPhotos.push(b.getAttribute('data-src') || ''); });
        lbShow(idx);
  ...
  lbEl.addEventListener('click', function(e) {
    if (e.target === lbEl) { lbEl.classList.remove('active'); document.body.style.overflow = ''; }
  });
  document.addEventListener('keydown', function(e) {
    if (!lbEl.classList.contains('active')) return;
    if (e.key === 'Escape') { lbEl.classList.remove('active'); document.body.style.overflow = ''; }
  ...
  document.querySelectorAll('.moment-video-play').forEach(function(btn) {
    ...
    var container = btn.closest('.moment-video') as HTMLElement;
    var video = container.querySelector('video') as HTMLVideoElement;
    video.addEventListener('play', function() { btn.classList.add('playing'); });
  ...
  document.querySelectorAll('.moment-actions .like-btn').forEach(function(btn) {
    btn.addEventListener('click', function() {
      ...
      if (btn.classList.contains('liked')) {
        ...
        setTimeout(function() { btn.classList.remove('like-pulse'); }, 350);
        if (momentName) {
