(function() {
  ...
  document.querySelectorAll('.site-nav nav a').forEach(function(link) {
    var href = (link as HTMLAnchorElement).getAttribute('data-href') || (link as HTMLAnchorElement).getAttribute('href') || '';
  ...
  function update() {
    ...
    if (hmsEl) hmsEl.textContent = String(hours).padStart(2, '0') + ':' + String(mins).padStart(2, '0') + ':' + String(secs).padStart(2, '0');
  }
  update();
  ...
  var navEl = document.querySelector('.site-nav nav') as HTMLElement;
  if (menuToggle && navEl) {
    menuToggle.addEventListener('click', function() {
      navEl.classList.toggle('mobile-open');
      var expanded = navEl.classList.contains('mobile-open');
